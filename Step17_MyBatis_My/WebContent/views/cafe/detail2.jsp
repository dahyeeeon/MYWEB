...
<!-- 댓글관련 UI -->
	...
	<c:forEach var="tmp" items="${list }">
			...
			<img src="${pageContext.request.contextPath}/resources/images/user_image.gif"/>
				...
				<c:if test="${tmp.num ne tmp.comment_group }">
					<br />
					<i class="muted">${tmp.target_id }</i>의 답글
				</c:if>
				<p>${tmp.content }</p>
				<form action="comment_insert.do" method="post">
				<!-- 덧글 작성자 -->
		...
		if(!isLogin){ //로그인 안했다면
			...
			if(isGoLogin){
				...
				location.href="${pageContext.request.contextPath}/users/loginform.do?url="
					+"${pageContext.request.contextPath}/cafe/detail.do?num=${dto.num}";
			}
			return false; //폼 전송 막기
		}
	});

	//답글 링크를 눌렀을때 실행할 함수등록
